...
bf3 = GBF_genGBF(G.U,G.Lambda,IntIndex,type3,alpha3,epsilon3);

s1 = GBF_itpGBF(bf1, IntIndex, y);
s2 = GBF_itpGBF(bf2, IntIndex, y);
s3 = GBF_itpGBF(bf3, IntIndex, y);
...
IntIndex = (1:10)';
y = zeros(size(IntIndex));
y(1:5) = ones(5,1);

%Calculate GBF interpolation of this second set with third and fourth kernel
bfvs = GBF_genGBF(G.U,G.Lambda,IntIndex,type3,alpha3,epsilon3);
bfband = GBF_genGBF(G.U,G.Lambda,IntIndex,type4,alpha4);
[svs,~] = GBF_itpGBF(bfvs,IntIndex,y);
[sband,~] = GBF_itpGBF(bfband,IntIndex,y);
